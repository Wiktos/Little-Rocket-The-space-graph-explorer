...
void ugraph_test::testDefauldConstructor() {
	...
	if (G.V() != 5) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "G.V() != 5");
	}

	if (G.E() != 0) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "G.E() != 0");
	}
}
...
void ugraph_test::testInitializerListConstructor() {
	...
	if (G.V() != 3) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "G.V() != 3");
	}

	if (G.E() != 3) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "G.E() != 3");
	}
}
...
void ugraph_test::testAdjMethod() {
	...
	if (adjOne != properOut) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "Wrong vertices adj to one");
	}
}
...
void ugraph_test::testAddEdgeMethod() {
	...
	if (adjOne != properOut) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "Wrong vertice added to one");
	}

	if (G.E() != 1) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "Wrong number of vertices");
	}

	try {
		G.addEdge(2, -2);
		ErrorMessenger::getInstance().report(ERROR_MSG + "exception should be thrown wrong idx");
	}
...
void ugraph_test::testDegreeMethod() {
	...
	if (UndirectedGraph::degree(G, 1) != 3) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "wrong degree of vertex 1");
	}

	if (UndirectedGraph::degree(G, 0) != 1) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "wrong degree of vertex 0");
	}
}
...
void ugraph_test::testMaxDegreeMethod() {
	...
	if (UndirectedGraph::maxDegree(G) != 3) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "max degree should be equal to 3");
	}
}
...
void ugraph_test::testNumberOfSelfLoopsMethod() {
	...
	if (UndirectedGraph::numberOfSelfLoops(G) != 1) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "number of self loops != 1");
	}
}
...
void ugraph_test::testAreNeighboursMethod() {
	...
	if(G.areNeighbours(2, 0)) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "2, 0 aren't neighbours");
	}

	if (!G.areNeighbours(2, 1)) {
		ErrorMessenger::getInstance().report(ERROR_MSG + "2, 1 are neighbours");
	}
}
